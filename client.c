 ...
 *		       Akshit Poddar <akshitpoddar@gmail.com>
...
main(int argc, char **argv)
{
	...
	for (ifihead = ifi = Get_ifi_info_plus( AF_INET, 1);
		...
		sock_desc[sock_count].subnet = (struct in_addr *)malloc( sizeof(struct in_addr));
		sock_desc[sock_count].subnet->s_addr =  sock_desc[sock_count].ip_addr->s_addr & sock_desc[sock_count].net_mask->s_addr;
		//priintf("ip = %d\n",sock_desc[sock_count].ip_addr->s_addr);
		//printf("net = %d\n",sock_desc[sock_count].net_mask->s_addr);
		a = sock_desc[sock_count].ip_addr->s_addr & sock_desc[sock_count].net_mask->s_addr;
		c =  sock_desc[sock_count].ip_addr->s_addr;
		d =  sock_desc[sock_count].net_mask->s_addr;
		//printf("a=%d\n", a);
		//sock_desc[sock_count].subnet->s_addr = a;
		//b.s_addr = (uint32_t)(c & d);
		 inet_ntop(AF_INET, &b /*sock_desc[sock_count].subnet*/, buf, MAXLINE);
		...
		printf("CLIENT: sock_desc[%d] SubNet Mask: %s\n", sock_count, buf /* Sock_ntop_host(sock_desc[sock_count].subnet, sizeof(struct in_addr))*/);
		printf("\n");
	...
	sleep(1);
	Sendto(sockfd, sendline, strlen(sendline), netopt, 0, 0);
        printf("CLIENT: passing Sendto()\n");
...
void * consumerThread(void *arg){
	...
	while(1){
		useconds_t sleep_time = -1*mean_time*log((double)(rand()*1.0/RAND_MAX));
		//printf("THREAD: %f\n", sleep_time);
		//sleep_time = 1;
		...
		Pthread_mutex_lock(&recv_buffer_lock);
		Pthread_cond_wait(&cond, &recv_buffer_lock);
		//printf("\nTHREAD WAKES UP\n");
